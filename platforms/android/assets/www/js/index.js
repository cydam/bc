...
var app = {
    ...
    receivedEvent: function(id) {// constants
				var TAILLE_ICONE = 40;
				var NB_LIGNES = 8;
				var NB_COLONNES = 8;
				var NB_ICONES = 4;
				...
				var soundUrl = "sounds/gvl_kojo.mp3";//7
				var xSPath = "sounds/explosion-04_2sec.mp3";
				var xSPathChute = "sounds/gvl_yeso.mp3";//6
				var xSPathFireball = "sounds/fire_2sec.mp3";
				var xSPathHyperC = "sounds/gvl_baba.mp3";//4
				var xSPathStarGem = "sounds/button-7.mp3";
				var xSPathEntrance = "sounds/drummer_boy.mp3";
				var xSPathGba = "sounds/gvl_gbasibe.mp3";
				var xSPathKojo = "sounds/gvl_kojo.mp3";
				var xSPathEnd = "sounds/gvl_nothing.mp3";//...we_end
				var info2Disp= "MOREEEE";
				var scoreFlag = 1;
				  ...
				  if (chaine_trouvee) {
					...
					if ($icone == undefined && $binome == undefined) {
					  if (test_possible_move()) {
						...
						if (score > 1000 && (score < 3999)) {
						  // difficulty++
						  NB_ICONES = 5;
						  if (scoreFlag==1){
						  DispInfo("1000+pts! Things are looking up for you");
						  return scoreFlag = 4;
						  }
						}
						else if (score > 4000 && (score < 6999)) {
						  // difficulty++
						  NB_ICONES = 6;
						  if (scoreFlag<=4){
						  DispInfo("4000+pts! You are good");
						  return scoreFlag = 6;
						  }
						}
						  ...
						  NB_ICONES = 8;
						  if (scoreFlag<=10){
						  DispInfo("10000+pts! You are a genius!");
						  return scoreFlag = 11;
						  }
						}
					...
					if (suite_horiz >= 3) {
					  ...
					  if (multiplier > 1) {
					   var $aff_combo = $('<div class="aff_combo" style="left:' + (colonne*TAILLE_ICONE) + 'px; top:' + (ligne*TAILLE_ICONE) + 'px;">x' + multiplier + '</div>');
						...
						PlaySound(xSPathGba); //this happens at auto combo
					  }
				...
				PlaySound(xSPathHyperC);
				$('#zone_message').append('<div class="hypercube">'+info+'</div>');
						  setTimeout(function () {
							$('#zone_message').html('');
						  }, 9000);
				}
